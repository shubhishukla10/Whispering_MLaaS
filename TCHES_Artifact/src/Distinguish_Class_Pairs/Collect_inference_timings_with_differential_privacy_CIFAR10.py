...
if args.model:
    ...
    if args.model == "alexnet":
        ...
        model.load_state_dict(torch.load(base_path+"Models/Alexnet_OpacusDP/cnn"))
    ...
    if args.model == "densenet":
        ...
        model = ModuleValidator.fix(model)
        ModuleValidator.validate(model, strict=False)
        model.load_state_dict(torch.load(base_path+"Models/Densenet_OpacusDP/cnn"))
        model = model.to(device)
        flag = 1
    if args.model == "resnet":
        ...
        model = ModuleValidator.fix(model)
        ModuleValidator.validate(model, strict=False)
        model.load_state_dict(torch.load(base_path+"Models/Resnet_OpacusDP/cnn"))
        model = model.to(device)
        flag = 1
    if args.model == "vgg":
        ...
        model = ModuleValidator.fix(model)
        ModuleValidator.validate(model, strict=False)
        model.load_state_dict(torch.load(base_path+"Models/VGG19_OpacusDP/cnn"))
        model = model.to(device)
        flag = 1
    if args.model == "squeezenet":
        ...
        model = ModuleValidator.fix(model)
        ModuleValidator.validate(model, strict=False)
        model.load_state_dict(torch.load(base_path+"Models/Squeezenet_OpacusDP/cnn"))
        model = model.to(device)
        flag = 1
    if args.model == "custom_cnn":
        ...
        model.load_state_dict(torch.load(base_path+"Models/CustomCNN_OpacusDP/cnn"))
        flag = 1
    ...
    if flag:
        ...
        for x_i in range(10):
            ...
            df.to_csv(base_path+'Timing_Data/CIFAR10/'+args.model+'_DP/Full_Function/Class_'+str(x_i)+'.csv')
        print("Finished!")
    else:
