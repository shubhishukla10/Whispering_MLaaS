...
if args.model in model_list:
        ...
        if args.dp == "yes":
                data_path = base_path+"Timing_Data/CIFAR100/"+args.model+"_DP/Full_Function/"
        else:
                data_path = base_path+"Timing_Data/CIFAR100/"+args.model+"/Full_Function/"

        for c_i in range(100):
                file1 = pd.read_csv(data_path +'Class_'+ str(c_i) +'.csv')

                for c_j in range(c_i+1,100):
                        file2 = pd.read_csv(data_path +'Class_'+ str(c_j) +'.csv')
                        dist1 = file1['Time'].to_numpy()        # dist1 stores timing values of Class i
                        dist2 = file2['Time'].to_numpy()        # dist2 stores timing values of Class j
                        ...
                        # Perfrom t-test on timing distributions of Class i and j
        ...
        # Store t-test results for all class pairs of CIFAR-100
        ...
        for i in range(4950):
                if dfStat_t['Statistic'][i] >= 4.5 or dfStat_t['Statistic'][i] <= -4.5:
                        cnt+=1
        print("Total distinguishable pairs (out of 4950) for "+args.model+" model: "+ str(cnt))
else:
